...
struct GameContext((u32, u32), Mutex<Option<Canvas<Window>>>, TextureCreator<WindowContext>);
unsafe impl Send for GameContext {}
unsafe impl Sync for GameContext {}

pub type InitializationContext = (CanvasAdapter, WindowSize, EventManager, Cursor, ResourceFacade<'static>);

lazy_static! {
    static ref sdl_contexts: SdlInitializationContext = {
        Bootstrapper::initialize_sdl().unwrap()
    };

    static ref game_context: GameContext = {
        Bootstrapper::initialize_game().unwrap()
    };
}
...
impl Bootstrapper {
    ...
    fn initialize_game() -> Result<GameContext, Box<Error>> {
        ...
        Ok(GameContext(window_size, Mutex::new(Some(canvas)), texture_creator))
    }

    pub fn initialize() -> Result<InitializationContext, Box<Error>> {
        let sdl_context = &sdl_contexts.0;
        let font_context = &sdl_contexts.1;

        let window = create_window(&sdl_context)?;
        let window_size = WindowSize(window.size());
        
        let canvas_adapter = CanvasAdapter::new(game_context.1.lock().unwrap().take());
